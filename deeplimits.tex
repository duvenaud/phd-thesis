...
%
with fixed%
\footnote{The above derivation gives the same result if the parameters of the hidden units are random.
However, to avoid confusion, we refer to layers with infinitely-many nodes as``fixed'', since their distribution on outputs always the same with probability one.
}
