...
\begin{figure}[t]
\begin{tabular}{c|c}
a) One-hidden-layer MLP & b) Two-hidden layer MLP \\
\null\hspace{-0.25cm}
...
\def\halfshift{0.65cm}

\begin{figure}[t]
\begin{tabular}{c|c}
%\multicolumn{2}{c}{} \\
%\multicolumn{2}{c}{
c) Two-layer \gp{}: $\vy = \vf^{(2)} \left( \vf^{(1)}(\vx) \right)$ & \\
%\multicolumn{2}{c}{
\begin{tikzpicture}[shorten >=1pt,->,draw=black!50, node distance=\layersep]
    \tikzstyle{every pin edge}=[<-,shorten <=1pt]
    \tikzstyle{neuron}=[circle,fill=black!25,minimum size=17pt,inner sep=0pt]
...
\begin{tikzpicture}[draw=black, node distance=\layersep]
    ...
    \node[annot,above of=H-1, node distance=\upnodedist, text width = 1cm] {Hidden Layer};
    \node[annot,above of=H2-1, node distance=\upnodedist, text width = 1cm] {Hidden Layer};
    \node[annot,below of=H-\numhidden, node distance=0.5cm, text width = 1cm] {$\vf^{(1)}(\vx)$};
    \node[annot,below of=H2-\numhidden, node distance=0.5cm, text width = 1cm] {$\vf^{(2)}(\vf^{(1)}(\vx))$};
    \node[annot,above of=O-1, node distance=\upnodedist] {Outputs};
    \node[annot,below of=O-\numdims, node distance=2.5cm, text width = 1cm] {$\vy$};
\end{tikzpicture}
%\end{tabular}
...
\end{align}
%
The parameter $\sigma^2$ controls the variance of functions drawn from the prior, and the parameter $\lengthscale$ controls the smoothness.  
