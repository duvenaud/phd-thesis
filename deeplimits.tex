...
In this section, we show the precise relationship between Gaussian processes and neural nets, then show two equivalent ways of constructing neural networks which give rise to deep \gp{}s.
...
\subsubsection{Random versus Fixed Layers}

%In the previous section we derived \gp{}s as neural nets having fixed hidden units.
Confusingly, the derivation of \citet{neal1995bayesian} uses random hidden units instead of fixed hidden units.
...
\centering
\begin{tabular}{cc}
No transformation: $p(\vx)$ & 1 Layer: $p(\vf^{(1)}(\vx))$ \\
\gpdrawbox{1} & \gpdrawbox{2} \\
4 Layers: $p(\vf^{(1:4)}(\vx))$ & 6 Layers: $p(\vf^{(1:6)}(\vx))$ \\
...
\newcommand{\mappic}[1]{\hspace{-0.05in}\includegraphics[width=0.475\columnwidth]{\deeplimitsfiguresdir/map/latent_coord_map_layer_#1}} 
\newcommand{\mappiccon}[1]{\hspace{-0.05in}\includegraphics[width=0.475\columnwidth]{\deeplimitsfiguresdir/map_connected/latent_coord_map_layer_#1}}
\begin{figure}
\centering
\begin{tabular}{cc}
...
This means that $\vf$ is unlikely to be a suitable representation for decision tasks that depend on more than one aspect of $\vx$.  Also note that the overall shape of the mapping remains the same as the number of layers increase -- for example, the roughly circular shape remains in the top-left corner even after 40 independent warpings.}
\label{fig:deep_map}
\end{figure}
%
...
\centering
\begin{tabular}{cc}
3 Connected layers: $p( f_C^{(1:3)}(\vx)$ & 6 Connected layers: $p( f_C^{(1:6)}(\vx)$ \\
\gpdrawboxcon{3} &
\gpdrawboxcon{6}
\end{tabular}
...
\mappic{0} & \mappiccon{2} \\
 10 Connected layers & 20 Connected layers \\
\mappiccon{10} & \mappiccon{20}
\end{tabular}
...
\citep{lawrence2007hierarchical}, presented a deep, semi-latent variable model called ``hierarchical \gplvm{}s'', in which time was mapped trough a composition of multiple \gp{}s to produce observations.
...
%\citet{damianou2012deep} developed a variational inference scheme for this model, analyzed the effect of automatic relevance determination, and showed that it could learn with relatively little data.
I submit that this model might be more clearly described as a ``deep \gplvm{}'', and that the term ``deep Gaussian process'' should be reserved for describing compositions of functions drawn from \gp{} priors.
...
\citet{wilson2012gaussian} introduced Gaussian process regression networks, which are defined as a matrix product of \gp{}s, rather than a composition.
...
\end{align}
%
We can easily define a ``deep'' Gaussian process regression network %\gprn{}:
%
\begin{align}
