...
This chapter also presents a system which generates reports combining automatically-generated text and plots which highlight interpretable features discovered in a data sets.
...
The paper (upon which this chapter was based) was written mainly by James Lloyd and myself.%, and Zoubin Ghahramani, with many helpful contributions and suggestions from Roger Grosse and Josh Tenenbaum.
...
In order to be able to use the same phrase to describe the effect of each base kernel in different circumstances, we convert each kernel expression into a standard, simplified form.
...
After applying these rules, any composite kernel in our language can be written as a sum of terms of the form:
\begin{align}
...
In order to build a noun phrase describing a product of kernels, we choose one kernel to act as the head noun, which is then modified by appending descriptions of the other kernels in the product.
...
%  \item We incorporate rules for which kernel is chosen as the head noun can change the interpretability of a description.
  ...
  \item Descriptions can include extra information about the model not contained in the kernel.
        For example, based on the slope of the posterior mean, we might write ``a linearly increasing function'' as opposed to ``a linear function''.
...
This section shows an example of our description procedure describing a compound kernel containing every type of base kernel in our set:
...
These describe models produced by the automatic search method presented in \cref{ch:grammar}.
...
Next, we present excerpts of the description generated by our procedure on a model of international airline passenger counts over time. %, shown in \cref{fig:airline}.
...
Combined with the model search of \cref{ch:grammar}, this gives a procedure combining all the elements potentially necessary for an automatic statistician listed in \cref{sec:ingredients}:
