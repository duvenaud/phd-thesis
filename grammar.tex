...
At each stage, we choose the highest scoring kernel and expand it by applying all possible operators:
%
\begin{center}
\begin{tabular}{rccc}
\textrm{Replacement} & $\kernel$ & $\to$ & $\kernel'$\\% & $\forall\, \kernel' $\\
\textrm{Addition} & $\kernel$ & $\to$ & $\kernel + \kernel'$\\% & $\forall\, j,\kernel' $\\
\textrm{Multiplication} & $\kernel$ &  $\to$ & $\kernel \times \kernel'$\\% & $\forall\, j,\kernel'$\\
\end{tabular}
\end{center}
%
We lso include additional operators to incorporate changepoints.
...
\Cref{fig:mauna_grow} shows the model increasing in sophistication as the kernel expression grows.}
...
Choosing kernel structures requires a criterion for comparing models.
