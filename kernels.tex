...
We'll also show several ways to encode group invariants into kernels.
...
In this chapter, $\InputSpace$ is usually a Euclidean space, but it could just as easily corresponding to the space of images, document, categories or points on a sphere.
...
Below, we'll focus on two ways of combining kernels: addition and multiplication.
...
\end{figure}
%
\Cref{fig:kernels_times} shows some interesting kernels that one can obtain by multiplying two basic kernels together.

Working with kernels rather than the parametric form of the function itself allows us to express high-level properties of functions that don't necessarily have a simple parametric form.
Here, we'll discuss a few examples.

\subsubsection{Polynomial Regression}
By multiplying together $T$ linear kernels, we obtain a prior on polynomials of degree $T$.
Column 1 of \cref{fig:kernels} shows a quadratic kernel.

\subsubsection{Locally Periodic Functions}
In univariate data, multiplying a kernel by \kSE{} gives a way of converting global structure to local structure.
For example, $\Per$ corresponds to exactly periodic structure, whereas $\Per \kerntimes \SE$ corresponds to locally periodic structure, as shown in column 2 of \cref{fig:kernels}.

\subsubsection{Functions with Growing Amplitude}
...
Columns 3 and 4 of \cref{fig:kernels} show two examples.

\subsubsection{More Combinations}
...
We'll see examples of real datasets with this kind of structure in \cref{ch:grammar}.
...
For example, products of $\kSE$ kernels, having a different lengthscale parameter for each dimension, are called the $\seard$ kernel:
%
\begin{align}
\seard( \vx, \vx')
...
\hspace{-0.2cm}\includegraphics[width=0.2\textwidth]{\additivefigsdir/2d-kernel/additive_kernel_sum_p1} 
& \hspace{-0.4cm} \raisebox{1cm}{$\times$} \hspace{-0.2cm} & 
\hspace{-0.2cm}\includegraphics[width=0.2\textwidth]{\additivefigsdir/2d-kernel/additive_kernel_sum_p2} \hspace{-0.4cm}
& \hspace{-0.4cm} \raisebox{1cm}{=} \hspace{-0.4cm} & \hspace{-0.4cm}
\includegraphics[width=0.2\textwidth]{\additivefigsdir/2d-kernel/sqexp_kernel} 
& \hspace{-0.4cm} \raisebox{1cm}{$\rightarrow$} \hspace{-0.4cm} & \hspace{-0.4cm}
...
$\kSE_1(x_1, x_1')$ & \hspace{-0.4cm} $\kerntimes$ & $\kSE_2(x_2, x_2')$ &  $=$ & \hspace{-0.4cm} $\kSE_1 \kerntimes \kSE_2$ & & \hspace{-0.5cm} \parbox{0.24\columnwidth}{$f(x_1, x_2)$ drawn from $\GPt{0}{\kSE_1 \kerntimes \kSE_2}$} \\
\end{tabular}
...
In addition, there is a theoretical reason to use them: they are \emph{universal} kernels~\citep{micchelli2006universal}, capable of learning learning any continuous function given enough data, under some conditions.
...
In general, the more structure we account for in the data, the less data we'll need - the \emph{blessing of abstraction} \citep{goodman2011learning} counters the curse of dimensionality.
Below, we'll investigate ways to encode more structure into our kernels.
...
\centering
\begin{tabular}{ccccc}
%\multicolumn{5}{c}{A sum of two orthogonal one-dimensional kernels}\\
...
\parbox{0.33\columnwidth}{$f(x_1, x_2) = \sin(x_1) + \sin(x_2)$} & $k_1(x_1, x_1') \kerntimes k_2(x_2, x_2')$ & $k_1(x_1, x_1') \kernplus k_2(x_2, x_2')$ \\[0.2em]
...
\end{figure}
%
\Cref{fig:synth2d} compares the extrapolations made by additive versus non-additive \gp{} models, conditioned on data from a sum of two axis-aligned sine functions, evaluated in a small, L-shaped area.
...
To illustrate how additive kernels give rise to interpretable models, we'll build an additive model of the strength of concrete as a function of the amount of 7 different ingredients, plus the age of the concrete \citep{yeh1998modeling}.
...
\raisebox{1cm}{f(x)} \hspace{-0.4cm} & \kernpic{../changepoint/draw_1} & \kernpic{../changepoint/draw_2} & \kernpic{../changepoint/draw_3} & \kernpic{../changepoint/draw_4} \\
& $x$ & $x$ & $x$ & $x$
\end{tabular}
...
The Cartesian product of the features of two one-dimensional $\kSE$ kernels covers the plane with two-dimensional radial-basis functions of the form
%
\begin{align}
...
In this section, we'll look at different ways in which we can encode symmetries into a prior on functions.
...
By using these functions to define warpings from $\Reals^2 \to \Reals^3$, we'll show how to build a nonparametric prior on an open-ended family of topological manifolds, such as cylinders, toruses, and M\"{o}bius strips.

\citet{ginsbourger2012argumentwise} and \citet{Invariances13} characterized the set of \gp{} priors on functions which respect any given invariance.
...
Formally, given a finite group of operations $G$ to which we wish our function to remain invariant, and $f \sim \GPt{0}{k(\vx,\vx')}$, $f$ is invariant under $G$ (up to a modification) if and only if $k(\cdot, \cdot)$ is argument-wise invariant:
%
\begin{align}
...
Ryan P. Adams (personal communication) suggests a construction enforcing invariants through products over the orbits:
%
\begin{align}
k_\textnormal{sum}(\vx, \vx') = \prod_{g, \in G} \prod_{g' \in G} k( g( \vx ), g'( \vx') )
\end{align}
%
This method can sometimes produce \gp{} priors with zero variance in some regions of the space, as in \cref{fig:add_vs_min}(c).
...
Another simple example of an easily-enforcable symmetry is symmetry about zero:
%
\begin{align}
f(x) = f( -x)
\end{align}
...
In contrast, we use translation invariance to refer to a the situation where the a signal has been discretized, and each pixel (or the audio equivalent) corresponds to a different input dimension.
We are interested in creating priors on functions that are invariant to swapping pixels in a manner that corresponds to shifting a in some direction:
%
\begin{align}
...
Priors on functions exhibiting symmetries can be used to create a prior on topological manifolds, by warping a latent surface $\vx$ to an observed surface $f(\vx)$.
...
To build a prior on 2-dimensional manifolds embedded in 3-dimensional space, we simply need a prior on mappings from $\mathbb{R}^2$ to $\mathbb{R}^3$, which we can construct using 3 independent functions $[f_1(\vx), f_2(\vx), f_3(\vx)]$ mapping from $\mathbb{R}^2$ to $\mathbb{R}$, specified by \gp{} priors.
...
A flexible way to construct a kernel over categorical variables is simply to represent that variable through a set of binary variables, using a one-of-k encoding.
For example, if $\vx$ can take one of 4 values, $x \in \{ \texttt{A}, \texttt{B}, \texttt{C}, \texttt{D}\}$, then a one-of-k encoding of $\vx$ will correspond to 4 binary inputs, and $\oneofk(\texttt{C}) = [0, 0, 1, 0]$.
...
A more flexible parameterization suggested by Kevin Swersky (personal communication) allows complete flexibility about which pairs of categories are similar to one another, replacing the $\seard$ kernel with a fully-parameterized kernel, $\sefull$:
%
\begin{align}
\sefull( \vx, \vx') = \exp \left( -\frac{1}{2} \vx\tra \vL \vx' \right)
\end{align}
%
where $L$ is symmetric.  This kernel individually parameterizes the covariance between each pair of function values of categories.
...
In the next chapter, we'll see how to perform such a search not just over the kernel paramaters, but also over the open-ended space of kernel expressions.

\subsubsection{Source Code}
Source code to produce all figures is available at \url{github.com/duvenaud/pdh-thesis}.
