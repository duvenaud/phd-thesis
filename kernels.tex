...
We will a real datasets with this kind of structure in \cref{ch:grammar}.
...
Additive noise can simply be modeled as another unknown function added to the signal.
This is typically acheived using a local kernel such as \kSE{} with a short lengthscale, as in the fourth column of \cref{fig:kernels_plus}.
The limit of a local kernel as its lengthscale goes to zero is a ``white noise'' (\kWN) kernel.
Draws from a \gp{} with a $\kWN$ kernel are simply \iid draws from a Gaussian random variable.
...
The formula for Gaussian conditionals, \cref{eq:gauss_conditional}, can be used to give the conditional distribution of a \gp{}-distributed function conditioned on its sum with another \gp{}-distributed function:
%
\begin{align}
...
For example, the squared-exponential kernel ($\kSE$) on the real line has a representation in terms of infinitely many radial-basis functions of the form ${h_i(x) \propto \exp( -\frac{1}{2} \frac{(x - c_i)^2}{2\ell^2})}$.
