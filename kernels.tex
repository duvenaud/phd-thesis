...
Column 1 of \cref{fig:kernels_times} shows a quadratic kernel.
...
For example, $\Per$ corresponds to exactly periodic structure, whereas $\Per \kerntimes \SE$ corresponds to locally periodic structure, as shown in column 2 of \cref{fig:kernels_times}.
...
Columns 3 and 4 of \cref{fig:kernels_times} show two examples.
...
For example, if $\vx$ can take one of 4 values, $x \in \{ \texttt{A}, \texttt{B}, \texttt{C}, \texttt{D}\}$, then a one-of-k encoding of $x$ will correspond to 4 binary inputs, and $\oneofk(\texttt{C}) = [0, 0, 1, 0]$.
