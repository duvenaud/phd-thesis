...
Top left: An additive kernel is simply a sum of kernels.
...
Top right: a product kernel is a product of kernels.
Bottom right:  A draw from a \gp{} prior with a product of kernels does not correspond to a product of draws from \gp{}s.
...
We model a dataset measuring the compressive strength of concrete, as a function of the concentration of 7 ingredients, plus the age of the concrete \citep{yeh1998modeling}.

We build a model
%
\begin{align}
f(\vx) & = 
f_1(\textnormal{cement})
+ f_2(\textnormal{slag})
+ f_3(\textnormal{fly ash})
+ f_4(\textnormal{water}) \\
& + f_5(\textnormal{plasticizer})
+ f_6(\textnormal{coarse})
+ f_7(\textnormal{fine})
+ f_8(\textnormal{age})
+ \epsilon
\label{eq:concrete}
...
We can see that two of the components
...
For example: given an additive model
%
\begin{align}
f(x) & = f^{(\textnormal{long})}(x) + f^{(\textnormal{short})}(x) \\
f^{(\textnormal{long})} & \sim \GP( \vzero, \kSE^{(\textnormal{long})} ) \\
f^{(\textnormal{short})} & \sim \GP( \vzero, \kSE^{(\textnormal{short})} )
\end{align}
%
we may be uncertain about which 
...
This reflects an ambiguiuty in the model - one of these functions is high and the other low, but which one is uncertain.
Dimensions `Coarse' and `Fine' are not shown here because their variance was zero.
}
...
We can see that there is negative correlation between the ``Cement'' and ``Slag'' variables.
This reflects an ambiguiuty in the model about which one of these functions is high and the other low.
...
Figure \ref{fig:synth2d} compares the extrapolations made by additive versus non-additive \gp{} models.
We constructed a training set by evaluating a sum of two axis-aligned sine functions in a small, L-shaped area.
In this example, the additive model is able to correctly predict the value of the function for unseen combinations of inputs.
The model using a product kernel is more flexible, and so remains uncertain about the function value at unseen combinations of inputs.
