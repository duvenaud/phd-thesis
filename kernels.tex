...
One can also compute the posterior covariance between any two components, conditioned on their sum:
%
\begin{align}
...
Different \gp{} priors on these functions will implictly give rise to different priors on warped surfaces.
...
A simple and flexible construction multiplies the original kernel $k(\vx, \vx')$ with a categorical kernel on the output index~\citep{bonilla2007multi}:
%
\begin{align}
...
Similarily, sometimes appropriate kernel structure is hard to guess.
