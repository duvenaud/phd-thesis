...
A kernel (also called a covariance function, kernel function, or covariance kernel), is a positive-definite function between two points $\inputVar, \inputVar'$ in some space $\InputSpace$. Formally, $\kernel(\vx, \vx'): \InputSpace \times \InputSpace \to \Reals$.
In this chapter, $\InputSpace$ is usually a Euclidean space, but it could just as easily correspond to the space of images, document, categories, or points on a sphere.
...
To remove clutter, we'll usually refer to kernels without specifying their parameters.
...
In general, the more structure we account for, the less data we'll need - the \emph{blessing of abstraction} \citep{goodman2011learning} counters the curse of dimensionality.
Below, we'll investigate ways to encode more structure into kernels.
...
To illustrate how additive kernels give rise to interpretable models, we'll build an additive model of the strength of concrete as a function of the amount of seven different ingredients, plus the age of the concrete \citep{yeh1998modeling}.
...
\citet{ginsbourger2012argumentwise} and \citet{Invariances13} showed that the only way to construct a \gp{} prior on functions which respect a given invariance is to construct a kernel which respects the same invariance with respect to each of its two inputs.
