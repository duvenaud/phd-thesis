...
%Hyperparameter learning in this model can be seen as Bayesian Hierarchical Kernel Learning (HKL).  
...
%This model is also extremely simple to implement, and we provide example code.
...
Because the variance parameters can specify which degrees of interaction are important, the additive GP is an extremely general model.  
...
If this is not the case, the kernel parameters can specify a suitably flexible model.
...
%Any one-dimensional base kernel is allowable, and each dimension can use a different base kernel.  One could even specify a mixture of base kernels for each dimension, allowing the parameters to specify the mixture weights.
...
%[Todo:  read and mention  and \citep{friedman1981projection} from Hannes]
...
\hspace{-0.06in} \includegraphics[height=0.35\textwidth]{\additivefigsdir/compare_models/hkl.pdf} \hspace{-0.07in} &
\hspace{-0.06in} \includegraphics[height=0.35\textwidth]{\additivefigsdir/compare_models/gam.pdf} \hspace{-0.07in} \\ \hline \\
 Squared-exp GP & Additive GP kernel\\
\hspace{-0.06in} \includegraphics[height=0.35\textwidth]{\additivefigsdir/compare_models/ard.pdf} \hspace{-0.07in} &
\hspace{-0.06in} \includegraphics[height=0.35\textwidth]{\additivefigsdir/compare_models/additive.pdf} \\
\end{tabular}
...
However, this method suffers considerably from the fact that cross-validation must be used to set kernel parameters.
...
\citet{DBLP:journals/corr/abs-0909-0844} also fixes the relative weighting between orders of interaction with a single term $\alpha$, computing the sum over all orders by:
\begin{equation}
\label{eqn:uniform}
...
The main difficulty with the approach of \citep{DBLP:journals/corr/abs-0909-0844} is that the kernel parameters are hard to set other than by cross-validation.
...
However, they recommend approximating the sum over all $D$ orders with only one term ``of appropriate order'', presumably because of the difficulty of setting the parameters of an \SVM{}.
...
\subsubsection{Overfitting and Parameterization}
...
For all \gp{} models, we fit kernel parameters by the standard method of maximizing training-set marginal likelihood, using L-BFGS \citep{nocedal1980updating} for 500 iterations, allowing five random restarts.
...
This dataset is one which can be predicted well by a kernel which is a sum of two-way interactions over the first 4 variables, ignoring the extra 4 noisy copies.%, as well as the higher-order interactions.
...
Additive GP performance can be expected to benefit from integrating out the kernel parameters.
...
In addition to improving modeling efficacy, the additive \gp{} also improves model interpretability:  the variance parameters on each order of intercation indicate which sorts of structure are present in our model.
