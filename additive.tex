...
\subsection{Additive Kernels can be Efficiently Evaluated}
...
\subsection{Dropout on Infinitely-Many Hidden Units has No Effect}
...
\end{figure}
%
This method chooses among \emph{hull} of kernels, defined as a set of terms such that if $\prod_{j \in J} k_j(\vx, \vx')$ is included in the set, then so are all lower-order terms containing the same elements: $\prod_{j \in J / i} k_j(\vx, \vx')$, for all $i \in J$.
