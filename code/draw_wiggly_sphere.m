...
kernel = @(x,y)(sphere_kernel(x,y) + per_kernel_1d(x', y'));
...
function sigma = per_kernel_1d(x, y)
    ...
    period = 0.05;
    lengthscale = 10;
    sigma = 0.001.*exp( -2.*sin(pi*(sqrt(sq_dist(x(2,:), y(2,:))))./period).^2 ./ lengthscale);
end
...
function sigma = sphere_kernel(x, y)
    if nargin == 0
        sigma = '2d Spherical covariance.'; return;
    end

    % Project x onto surface of sphere.
    xs(:, 1) = sin(x(:,1).*2*pi).*cos(x(:,2).*2*pi);
    xs(:, 2) = sin(x(:,1).*2*pi).*sin(x(:,2).*2*pi);
    xs(:, 3) = cos(x(:,1).*2*pi);
    
    % Project y onto surface of sphere.
    ys(:, 1) = sin(y(:,1).*2*pi).*cos(y(:,2).*2*pi);
    ys(:, 2) = sin(y(:,1).*2*pi).*sin(y(:,2).*2*pi);
    ys(:, 3) = cos(y(:,1).*2*pi);    
    
    sigma = exp( -0.5.*sq_dist(xs', ys') ./ 10);
end
